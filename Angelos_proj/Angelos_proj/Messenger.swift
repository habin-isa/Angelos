...
class Messenger {
    ...
    let messages = ["PLZ SEND HELP NOW", "Call the Police", "You have been listed as the emergency contact"];
...
//        sendMessageHelper(body: body)
    ...
    func sendEmergencyContactMessage(phoneNumber:String, alamo:Alamo = Alamo()) -> Void {
        let body = messages[2]
        let url = "https://api.twilio.com/2010-04-01/Accounts/\(accountSID)/Messages"
        let parameters = ["From": "+441423740326", "To": phoneNumber, "Body": body]
        alamo.request(url: url, parameters: parameters, accountSID: accountSID, auth: auth)
